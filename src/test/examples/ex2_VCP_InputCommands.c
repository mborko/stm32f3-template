/*#############################################################
  File Name       : ex2_VCP_InputCommands.c
...
int main(void)
{
	...
	char outbuf[40], inbuf[40];																		//string variable of 40 characters
	...
	sprintf(outbuf, "Enter your name: ");													//print characetrs to the buf string
	VCP_PutStr(outbuf);																						//write a string variable to the VCP
	VCP_GetStr(inbuf);																						//wait for the user to enter a string via the COM port from the PC (terminated by \n)

	sprintf(outbuf, "Hallo %s\n", inbuf);
	VCP_PutStr(outbuf);
